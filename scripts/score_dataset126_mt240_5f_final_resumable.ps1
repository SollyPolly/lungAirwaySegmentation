...
$FinalOut = Join-Path $Project 'data\nnunet\predict_out\Dataset126_val_mt240_5f_lungcrop_final_teacher'
$PartDir = Join-Path $FinalOut 'score_parts_lcc6'
$FinalJson = Join-Path $FinalOut 'nnunet126_val_mt240_5f_final_teacher_topology_lcc6.json'
...
if (-not (Test-Path -LiteralPath $FinalOut -PathType Container)) {
    throw "Final-teacher prediction directory is missing: $FinalOut"
}

$MissingPredictions = @(
    $Cases | Where-Object {
        -not (Test-Path -LiteralPath (Join-Path $FinalOut "ATM_$_.nii.gz") -PathType Leaf)
    }
)
if ($MissingPredictions.Count) {
    throw "Missing final-teacher prediction(s): $($MissingPredictions -join ', ')"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Merging final-teacher score parts failed with exit code $LASTEXITCODE."
}

Write-Host "Completed final-teacher score: $FinalJson" -ForegroundColor Green
